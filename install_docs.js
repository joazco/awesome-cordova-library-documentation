...
class InstallDocs {
        ...
        return new Promise((resolve, reject) => {
            libraries.forEach(library => {
                ...
                fs.access(fullPath, fs.constants.R_OK | fs.constants.W_OK, (err) => {
                    if (err) {
                        console.log(path.join("@awesome-cordova-library", library),
                            path.join("docs/plugins", library));
    ...
    exec() {
        ...
        this.checkReadMeFromLibrary().then((data) => this.checkReadMeToDocumentation(data)).catch(err => console.error(err))
    }
}
